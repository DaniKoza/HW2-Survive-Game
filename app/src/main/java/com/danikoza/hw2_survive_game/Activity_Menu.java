...
public class Activity_Menu extends AppCompatActivity {
    ...
    /* Changed find views so it will compile */
    ...
    private void makeServerCall() {
        new Thread() {
            public void run() {
                String data = getJSON(Activity_Menu.this.getString(R.string.url));
                Log.d("pttt", data);
                if (data != null) {
                    Activity_Menu activity_Menu = Activity_Menu.this;
                    activity_Menu.startGame(menu_EDT_id.getText().toString(), data);
                }
            }
        }.start();
    }
